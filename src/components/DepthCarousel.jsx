...
const DepthCarousel = ({
  ...
  const layout = useCallback((pos) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const brightness = Math.max(0.85, 1 - back * (cfg.falloff * 0.35));
      ...
      if (ov) ov.style.opacity = clamp(back * cfg.falloff * 0.25, 0, 0.12).toFixed(3);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      const cfg = cfgRef.current;
      const needed = cfg.cardWidth + Math.abs(cfg.spread) * 1.1 + 20;
  ...
  return (
    <div
      ref={rootRef}
      className={`depth-carousel-root ${className}`.trim()}
      ...
      onKeyDown={onKeyDown}
    >
      <div className="depth-carousel-stage" ref={stageRef}>
        {data.map((item, i) => (
          <div
            key={i}
      ...
      {showControls && count > 1 && (
        <>
          <button
            type="button"
            className="depth-carousel-btn depth-carousel-prev"
          ...
          <button
            type="button"
            className="depth-carousel-btn depth-carousel-next"
      ...
      {showIndicators && count > 1 && (
        <div
          className="depth-carousel-indicators"
          role="tablist"
          aria-label="Slides"
        >
          {data.map((_, i) => (
            <button
              ...
              className={`depth-carousel-dot ${active === i ? 'active' : ''}`}
